...
def load_data():
    """Load products, queries, and query-grouped splits."""
    ...
    if not required_product_columns.issubset(products.columns):
        ...
        raise ValueError(f"Products missing columns: " f"{sorted(missing)}")

    if not required_query_columns.issubset(queries.columns):
        missing = required_query_columns - set(queries.columns)
        raise ValueError(f"Queries missing columns: " f"{sorted(missing)}")
...
def evaluate_split(
    ...
    """
    Evaluate TF-IDF on a WANDS split.

    The candidate set for each query is the complete set
    of judged query-product pairs belonging to that query.

    The retriever returns the top-k candidates.

    If prediction_output_path is supplied, the actual
    top-k rankings are saved.
    """
    ...
    for query_id, group in split.groupby(
        ...
        if query_id not in query_lookup:
            raise ValueError(f"Query ID {query_id} " f"not found in queries.")

        query_text = str(query_lookup[query_id])

        # ---------------------------------------------------------
        # Add product names to the judgment candidates.
        # ---------------------------------------------------------
        ...
        if ranked is None or ranked.empty:
            raise ValueError(
                f"TF-IDF returned no candidates " f"for query_id={query_id}"
            )
        ...
        ranked_product_ids = ranked["product_id"].astype(int).tolist()
        ...
        # ---------------------------------------------------------
        # Save actual ranking predictions.
        # ---------------------------------------------------------
...
def main() -> None:
    print("=" * 70)
    print("RelevanceFlow — " "TF-IDF Retrieval Baseline")
    print("=" * 70)
    ...
    print(f"TF-IDF fit time: " f"{fit_time:.3f} seconds")
    ...
    metrics = {
        ...
        "evaluation_protocol": {
            ...
            "tfidf_fit_scope": ("training-associated " "products only"),
        },
        "config": {
    ...
    print("\nTF-IDF baseline " "completed successfully.")
